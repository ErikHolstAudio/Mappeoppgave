...
public:
    ...
    float getHeightFromIndex(int index);
    int mColumns;
    ///The height of the image, in pixels
    int mRows;
    ///Number of bytes describing each pixel. Has to be 3 or 4.

private:
    GLubyte pixels[16];     // For the standard texture from the no-parameter constructor
    ///Texture ID/name that OpenGL makes when glGenTextures is called
    GLuint mId{0};
